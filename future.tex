...
\begin{frame}{Research Direction}
  ...
  \begin{tikzpicture}
    ...
    \node[rounded corners, thick, draw] (dep) at (25mm, 0mm){Deployment};
    \node[rounded corners, thick, draw] (imp) at (0, 0) {Implement};
    \draw (ana) -- (meth) --(app) -- (und) -- (dep) -- (imp) -- (ana);
  \end{tikzpicture}
  \end{center}
\end{frame}

\begin{frame}{Method: Deep Latent-Variable Models}
...
\begin{frame}[fragile]{Interpretability}
