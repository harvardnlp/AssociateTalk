...
\begin{frame}{Deep Latent-Variable Models}
  ...
  Goal: Extent text generation to Expose specific choices as \textit{discrete} latent variables.
...
\end{enumerate}
\[p(y, z| x \param \theta)
       = \mu_{z} \times   \RNNLM(y_{1:T} \param \pi_z) \]j
% \[p(x, z \param \theta)
...
\begin{frame}{Application: Summary with Copy-Attention}{(Gu et al, 2016) (Gulcehre et al, 2016)}
...
\begin{frame}{Technical Methodology:  Neural Hidden Semi-Markov Model}
  ...
  \item Transition Distribution: NN between states.
    ...
    \air
  \item Emission Distribution: Seq2Seq+Attention, one per state $k$.
...
\begin{frame}{Reasoning-Based Models }
  \begin{center}
...
\begin{frame}{Long-Form Generation with Reasoning}
