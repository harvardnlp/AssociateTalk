...
\begin{frame}{Theoretical Issues with Multiclass Training for Generation}
  ...
  \item Exposure Bias
    \begin{itemize}
    \item Training conditions on true history ($y_{1:t-1}$) but generation uses predicted history.
    \end{itemize}
    \air
    \pause
...
\begin{frame}{Beam Search Optimization}
  Can we fix these issues for text generation?  
  \air
...
\begin{frame}{Modification 1: Beam Search at Training}
  ...
  \textbf{Issue:}  Exposure Bias
   ...
   \textbf{Proposed Fix:} During training, run until \textit{loss occurs}.
   \begin{enumerate}
   \item Compute the score of every possible next word.
...
\begin{frame}{Modification 2: Global Scoring Function  }
  ...
  \begin{itemize}
  \item Replace $ \log p(y_{t} | y^{(k)}_{1:t-1}, x; \theta)$ with
...
\begin{frame}{Modification 3.1: Train with Margin}
...
\begin{frame}{Modification 3.2: Train with Constraints}
\end{frame}
