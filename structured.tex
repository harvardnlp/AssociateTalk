...
\begin{frame}{Structured Modeling}
  Compare to multiclass.
\end{frame}
...
\begin{frame}{Training Seq2Seq}
  ...
  \end{center}
  Treat as Multiclass classification:
...
\begin{frame}{Theoretical Issues: Discrete Time-Series Moels  }
  \begin{enumerate}
  \item Exposure Bias
    \begin{itemize}
  ...
  \item Label Bias  %\cite{Lafferty2001}
    \begin{itemize}
    \item Locally normalized discriminative models have known pathological issues.
    \end{itemize}
...
\begin{frame}{ Change 3: Replace train to enforce beam-search margin}
  ...
  \item Margin between ground truth seq $y^{(g)}$ and last seq on beam $y^{(K)}$
  \end{itemize}
\begin{align*}
...
% \end{align*}
  ...
  \begin{itemize}
  \item Color \textcolor{yellow}{True}: target sequence $y^{(g)}$
