...
\visible<7->{
   \begin{enumerate}
   \item For each $k$ in beam, example all possible next words $y_t$,
     \[f_{\theta}(\langle y_t, y_{1:t-1}^{(k)}\rangle, x) \gets \log p(y_{t}\ |\ y^{(k)}_{1:t-1}, x) + \log p(y^{(k)}_{1:t-1} \ |\  x) \]
     \pause
   \item Prune ($K \times \text{vocab}$) expansions to top $K$,
...
\begin{frame}{Baseline: Word-Level Knowledge Distillation (Multiclass)}
