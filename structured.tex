...
\begin{frame}{Core Issues}
  ...
  \end{enumerate}
  \pause
  \textbf{Strategy:} Modify training to fix these issues. 
\end{frame}
...
\begin{frame}{Modification 3: Train with Margin}
  ...
  \[ \mathcal{L}(\theta) = \sum_{t} \Delta(\hat{y}_{1:t}, y_{1:t}^{(K)}) \left[1 - f_{\theta}( \hat{y}_{1:t}, x) +  f_{\theta}(y_{1:t}^{(K)}, x) \right] \]
  
  \begin{itemize}
  \item Positive if true sequence $\hat{y}_{1:t}$ within margin of worst beam sequence $y^{(K)}_{1:t}$
